...
class Trainer(UnetTrainer):
    ...
    def __init__(self, args, Loader, device, n_classes=1):
        ...
        :param target_data:
        '''
        super(Trainer, self).__init__(args, Loader, device, n_classes=n_classes)
        ...
        try: self.mean_pos = Loader.mean_pos
        except:
            pass
        self.Temperature = args.Temperature # temperature applied to teacher's output

        self.train_ver = 'v3'   # which version of training function
    ...
    def TrainOneEpoch_v1(self, writer=None, max_itr=np.inf):
        '''
        Train One Epoch for CutMix model
        Apply transformation to unlabeled image before mixup
        Apply forward and inverse transformation to mixed image
        :return:
        '''
        ...
        while True:
            ...
            if w != 0.0:
                train_unlabeled_data_viewtch, train_unlabeled_gt_viewtch, train_unlabeled_mask_viewtch, train_unlabeled_Tform_viewtch = \
            ...
            if w != 0.0:
                ...
                nUnlabel = train_unlabeled_data_viewtch.shape[0]
                CutMixMask = []
                H = train_unlabeled_data_viewtch.shape[2]
                W = train_unlabeled_data_viewtch.shape[3]
                CutBoxRange = [[H * W / 2 / W, H], [H * W / 2 / H, W]]
                MixPair = []
                train_unlabeled_data_viewstu = [] # student view
                train_unlabeled_gt_viewstu = [] # student view
                for p_i in range(nUnlabel):
                    ...
                    img = train_unlabeled_data_viewtch[p_i].copy()
                    img[:,corner_H:corner_H+box_H, corner_W:corner_W+box_W] = \
                        train_unlabeled_data_viewtch[p2_i,:,corner_H:corner_H+box_H, corner_W:corner_W+box_W]
                    train_unlabeled_data_viewstu.append(img)
                    # Mix up GT
                    gt = train_unlabeled_gt_viewtch[p_i].copy()
                    gt[corner_H:corner_H + box_H, corner_W:corner_W + box_W] = \
                        train_unlabeled_gt_viewtch[p2_i, corner_H:corner_H + box_H, corner_W:corner_W + box_W]
                    train_unlabeled_gt_viewstu.append(gt)

                train_unlabeled_data_viewstu = np.stack(train_unlabeled_data_viewstu)
                train_unlabeled_gt_viewstu = np.stack(train_unlabeled_gt_viewstu)

                ## Apply augmentation to student view
                train_unlabeled_data_viewstu, train_unlabeled_gt_viewstu, train_unlabeled_mask_viewstu, train_unlabeled_Tform_viewstu = \
                    self.ApplyAugmentation_Mask(np.einsum('ijkl->iklj',train_unlabeled_data_viewstu),
                                                train_unlabeled_gt_viewstu)

                train_unlabeled_data_viewtch = torch.tensor(train_unlabeled_data_viewtch,device=self.device, dtype=torch.float32)
            ...
            if w != 0.0:
                ...
                train_unlabeled_pred_viewtch = self.net_ema(train_unlabeled_data_viewtch)   # forward pass prediction of train unlabeled set view2

                ## Inverse transformation
                # train_unlabeled_pred_viewtch_aligned = self.gt_model.invtransform_image_tensor(train_unlabeled_pred_viewtch,
                #                                                                              train_unlabeled_Tform_viewtch)
                train_unlabeled_pred_viewtch_aligned = train_unlabeled_pred_viewtch

                train_unlabeled_pred_viewstu_aligned = self.gt_model.invtransform_image_tensor(
                    train_unlabeled_pred_viewstu,
                    train_unlabeled_Tform_viewstu)

                ## blend prediction map for mixed images
                train_unlabeled_pred_viewteachbld = []  # teacher view blended
                train_unlabeled_mask_viewteachbld = []  # teacher view blended
                for p_i in range(nUnlabel):
                    p2_i = MixPair[p_i][1]
                    blend = train_unlabeled_pred_viewtch_aligned[p_i].clone()
                    pred2 = train_unlabeled_pred_viewtch_aligned[p2_i]
                    ...
                    train_unlabeled_mask_viewteachbld.append(torch.tensor(train_unlabeled_mask_viewstu[p_i]*train_unlabeled_mask_viewtch[p_i] * train_unlabeled_mask_viewtch[p2_i],
                                                  dtype=torch.uint8,
                ...
                train_unlabeled_mask_viewteachbld = torch.stack(train_unlabeled_mask_viewteachbld)
                ...
                train_all_pred_allview_sel = [torch.masked_select(train_unlabeled_pred_viewteachbld, train_unlabeled_mask_viewteachbld),
                                              torch.masked_select(train_unlabeled_pred_viewstu_aligned, train_unlabeled_mask_viewteachbld)]
    ...
    def TrainOneEpoch_v2(self, writer=None, max_itr=np.inf):
        '''
        Train One Epoch for CutMix model
        Apply transformation to unlabeled image after mixup
        Apply forward and inverse transformation to mixed image
        :return:
        '''
        ...
        while True:
            ...
            if w != 0.0:
                ...
                # train_unlabeled_data_view2, train_unlabeled_gt_view2, train_unlabeled_mask_view2, train_unlabeled_Tform_view2 = \
                #     self.ApplyAugmentation_Mask(train_unlabeled_data, train_unlabeled_gt)
                train_unlabeled_data_viewtch = np.einsum('ijkl->iljk',train_unlabeled_data)
                train_unlabeled_gt_viewtch = np.einsum('ijk->ijk',train_unlabeled_gt)
            ...
            if w != 0.0:
                ...
                ## Apply augmentatoin to teacher view
                train_unlabeled_data_viewtch, train_unlabeled_gt_viewtch, train_unlabeled_mask_viewtch, train_unlabeled_Tform_viewtch = \
                        self.ApplyAugmentation_Mask(np.einsum('ijkl->iklj',train_unlabeled_data_viewtch),
                                                    train_unlabeled_gt_viewtch)
    ...
    def TrainOneEpoch_v3(self, writer=None, max_itr=np.inf):
        '''
        Train One Epoch for CutMix model
        Apply forward and inverse transformation to unlabeled image in teacher branch only
        :return:
        '''
        ...
        while True:
            ...
            if w != 0.0:
                ...
                train_unlabeled_pred_viewstu_aligned = train_unlabeled_pred_viewstu
                train_unlabeled_mask_viewstu = torch.squeeze(torch.ones_like(train_unlabeled_pred_viewstu_aligned)).detach().cpu().numpy()
                # train_unlabeled_pred_viewstu_aligned = self.gt_model.invtransform_image_tensor(
                #     train_unlabeled_pred_viewstu,
                #     train_unlabeled_Tform_viewstu)
    ...
    def TrainOneEpoch(self, max_itr=np.inf):
        ...
        if self.train_ver == 'v2':
            return self.TrainOneEpoch_v2(max_itr=np.inf)
        elif self.train_ver == 'v1':
            return self.TrainOneEpoch_v1(max_itr=np.inf)
        elif self.train_ver == 'v3':
            return self.TrainOneEpoch_v3(max_itr=np.inf)
