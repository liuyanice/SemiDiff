...
class Trainer(UnetTrainer):
    ...
    def EvalTrainSet(self, writer=None, max_itr=np.inf):
        ...
        while True:
            ...
            # train_unlabeled_data_view2, train_unlabeled_gt_view2, train_unlabeled_mask_view2, train_unlabeled_Tform_view2 = \
            #     self.ApplyAugmentation_Mask(train_unlabeled_data, train_unlabeled_gt)
            train_labeled_data_view1 = torch.tensor(train_labeled_data.transpose([0,3,1,2]),device=self.device)
            train_labeled_gt_view1 = torch.tensor(train_labeled_gt,device=self.device)
    ...
    #         loss = self.DiceLoss(pred=test_pred[:, 0, ...], gt=test_gt)
    #         epoch_loss = epoch_loss*itr/(itr+1) + loss.item()/(itr+1)
    #
    #         # accumulate predictions and ground-truths
    ...
    #     test_pred_bin_all = np.squeeze(test_pred_all > 0.5, axis=1).astype(float)  # binarize predictions
    ...
    def TestAll_SavePred(self,exp_fig=False,best_ckpt_filepath=None):
        ...
        if self.target_data == 'CrackForest' or self.target_data == 'MITRoad' or \
                self.target_data == 'MITRoadClean' or ('CHASEDB1' in self.target_data) or ('STARE' in self.target_data) or self.target_data == 'Gaps384' or \
                ('EM' in self.target_data) or ('DRIVE' in self.target_data):
