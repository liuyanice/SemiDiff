...
parser.add_argument('--Ckpt',type=str,help='Ckpt to resotre for test/inference',
                    default=r'C:\Users\liuyan\Desktop\给杨燕老师汇报\SemiDiff\Results\MeanTeacher_ResUnet_Q_Dice+ConsistMSE_loss_ep-1000_m-0.05_2023-06-27_10-56-48\ckpt')
...
parser.add_argument('--AddUnlab','-au',type=str ,help='Additional Unlabeled data [default: None] candidates: None, Crack500,Fake',default='Fake')
...
parser.add_argument('--SwapLabel','-sb',type=int ,help='Swap label 0 with 1 (It is not compatible with DiceLoss)[default: None]',default=0)
...
elif 'CHASEDB1' in args.TargetData:
    import DataIO_CHASEDB1 as DataIO
...
## Define Network
if 'PiMdl' in args.ssl or 'MeanTeacher' in args.ssl or 'VAT' in args.ssl or 'CutMix' in args.ssl:
...
if args.Mode == 'train':
    ...
    with torch.no_grad():
        ...
        loss_te, perpix_acc_te, macro_iou_te, micro_iou_te, AIU, micro_F1, macro_F1, macro_Precision, macro_Recall = Trainer.TestAll_SavePred(exp_fig=True)
    ...
    with torch.no_grad():
        ...
        if args.ExportFigure:
            ...
            loss_te, perpix_acc_te, macro_iou_te, micro_iou_te, AIU, micro_F1, macro_F1, macro_Precision, macro_Recall = Trainer.TestAll_SavePred(exp_fig=False)

        macro_miou_te = np.mean(macro_iou_te)
        macro_mF1 = np.mean(macro_F1)
        macro_mPrecision = np.mean(macro_Precision)
        macro_mRecall = np.mean(macro_Recall)
    ...
    print('Macro Average F1/test: {:.2f}%'.format(100 * macro_mF1))
    print('Micro Average F1/test: {:.2f}%'.format(100 * micro_F1))
    print('Macro Average Precision/test: {:.2f}%'.format(100 * macro_mPrecision))
    print('Micro Average Recall/test: {:.2f}%'.format(100 * macro_mRecall))
