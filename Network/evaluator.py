...
class evaluate():
    ...
    def F1(self, preds, gts):
        ...
        Precision_micro = metrics.precision_score(gts.reshape([-1]).astype(int), preds.reshape([-1]).astype(int))
        Recall_micro = metrics.recall_score(gts.reshape([-1]).astype(int), preds.reshape([-1]).astype(int))
        F1_macro = []
        Precision_macro = []
        Recall_macro = []
        for pred, gt in zip(preds, gts):
            F1_macro.append(metrics.f1_score(gt.reshape([-1]).astype(int), pred.reshape([-1]).astype(int)))
            Precision_macro.append(metrics.precision_score(gt.reshape([-1]).astype(int), pred.reshape([-1]).astype(int)))
            Recall_macro.append(metrics.recall_score(gt.reshape([-1]).astype(int), pred.reshape([-1]).astype(int)))

        F1_macro = np.mean(F1_macro)
        Precision_macro = np.mean(Precision_macro)
        Recall_macro = np.mean(Recall_macro)

        return F1_micro, F1_macro, Precision_macro, Recall_macro
...
class evaluate_list():
    ...
    def F1(self, preds, gts):
        ...
        Precision_micro = metrics.precision_score(gts_all.reshape([-1]).astype(int), preds_bin_all.reshape([-1]).astype(int))
        Recall_micro = metrics.recall_score(gts_all.reshape([-1]).astype(int), preds_bin_all.reshape([-1]).astype(int))
