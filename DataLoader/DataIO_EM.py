...
class DataIO():
    ...
    def __init__(self,batch_size, seed_split=0, seed_label=0, label_percent=0.05,
        ...
        self.fake_path = os.path.join(self.data_path,'samples')
    ...
    def LoadDataset(self):
        ...
        with open(os.path.join(self.data_path, 'train.txt'), 'r') as fid:
            ...
            for f_i in file_list:
                ...
                self.all_data.update(
                    {img_name: {'img': (255*img).astype(int), 'gt': seg, 'gt_org': None, 'name': img_name, 'index': data_idx}})
                data_idx += 1
                self.num_train += 1
        ...
        if self.add_unlab != 'None':
            # Load Training Data and cache in memory
          with open(os.path.join(self.data_path, 'fake.txt'), 'r') as fid:
            file_list = fid.readlines()
            fake_img_list = []
            fake_gt_list = []
            data_idx = 0
            for f_i in file_list:
                fake_img_list.append(f_i.rstrip())
                fake_gt_list.append(f_i.rstrip())
                # Load image and update all data dictionary
                img_name = fake_img_list[-1]
                img = cv2.resize(plt.imread(os.path.join(self.fake_path, fake_img_list[-1])),
                                 self.resize_image_size, interpolation=cv2.INTER_NEAREST)  # load image array
                seg = cv2.resize(plt.imread(os.path.join(self.fake_path, fake_gt_list[-1])),
    ...
    def GenerateSplit_EqLabUnlab(self, lab_ratio, seed=0):
        ...
        self.lab_ratio = lab_ratio  # ratio of labeled data within a mini-batch
